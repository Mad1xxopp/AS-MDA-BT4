...
let signer;
let userAddress;
const tokenAddress = "0x48c72c6D2856F45964f53655b221657EDDB92001";
...
async function checkConnection() {
  ...
  if (accounts.length === 0) {
    ...
    balanceElement.innerText = "0 MDA";
  } else {
    document.getElementById(
      "wallet-status"
    ).innerText = `Connected: ${accounts[0]}`;
    const balance = await tokenContract.balanceOf(accounts[0]);
    balanceElement.innerText = `${ethers.utils.formatEther(balance)} MDA`;
  }
}
...
async function updateTokenBalance() {
  try {
    ...
    document.getElementById(
      "tokenBalance"
    ).innerText = `Balance: ${ethers.utils.formatEther(balance)} MDA`;
  } catch (error) {
